...
while not rospy.core.is_shutdown():
    ...
    # disparity_map = disparity.compute(frame, frame_right)
    ...
    for obj in old_trackers:
        ...
        if detect.disparity.shape[0] > 0 and w*h != 0:
            ...
            pos_z = detect.f * detect.T / d_mean
            if math.isnan(pos_z):
                print detect.disparity[y:y+h, x:x+w].shape, d.shape, x, y, w, h
            pos_x = (obj['box'][0] + obj['box'][2]/2 - input_w/2) * pos_z / detect.f
            pos_y = (obj['box'][1] + obj['box'][3]/2 - input_h/2) * pos_z / detect.f
            pos_w = obj['box'][2] * pos_z / detect.f
            pos_h = obj['box'][3] * pos_z / detect.f
