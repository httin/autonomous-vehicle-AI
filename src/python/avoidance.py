...
def velocity_distance(obstacle, r_rob, pos, yaw):
    ...
    for t in range(0, 6):
        # r = obstacle[2] + r_rob
        if obstacle[3] <= 0.01 and obstacle[4] <= 0.01 and t > 0:
            break
        x = (obstacle[0] + obstacle[3]*t - pos[0])*math.sin(yaw) - (obstacle[1] + obstacle[4]*t - pos[1])*math.cos(yaw)
        y = (obstacle[0] + obstacle[3]*t - pos[0])*math.cos(yaw) + (obstacle[1] + obstacle[4]*t - pos[1])*math.sin(yaw)
...
avoid = Avoidance()
rate = rospy.Rate(20)
rospy.Subscriber('My_Obstacles', Float32MultiArray, avoid.subscribe_obj)
...
safety_margin = 0 # (m) Increase size of obstacles by a safety margin
...
while not rospy.is_shutdown():
    ...
    i_finalgoal = len(x_path) - 1
    step = 1 if i_curr<=i_finalgoal else -1
    world = avoid.obj
    detected = []
    for obj in world:
        x = obj[0]*math.sin(robot_yaw) + obj[1]*math.cos(robot_yaw) + robot_pos[0]
        y = - obj[0]*math.cos(robot_yaw) + obj[1]*math.sin(robot_yaw) + robot_pos[1]
        detected.append([x, y, obj[2], obj[3], obj[4]])
    ...
    if np.sqrt((robot_pos[0] - final_goal[0])**2 + (robot_pos[1] - final_goal[1])**2) > dist_goal:
        ...
        if detected_flag == 1:
            i_prevgoal = i_goal if i_goal is not None else None
            collision = []
            for obstacle in detected:
            ...
            for j in range(i_goal + step, i_finalgoal, step):
                ...
                if dist2 <= dist1 and dist2 <= dist3:
                    ...
                    break
            if i_prevgoal is not None and i_goal <= i_prevgoal:
                i_goal = i_prevgoal
